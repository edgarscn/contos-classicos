...
export const register = (username, email, password, guestStats = null) => {
  ...
  const emailExists = Object.values(accounts).some(acc => acc.email === cleanEmail);
  if (emailExists) {
    return { success: false, error: "Este e-mail já está cadastrado." };
  }
  
  // Merge guestStats or create new empty stats
  const initialStats = guestStats ? { ...guestStats } : {
    readStories: {},
    streak: 0,
    longestStreak: 0,
    lastReadDate: "",
  ...
  localStorage.setItem(`reading_stats_${cleanUsername}`, JSON.stringify(initialStats));
  
  // Log user in
  setSession(cleanUsername);
  
  return { success: true, user: { username: username.trim(), email: cleanEmail } };
};
...
export const login = (usernameOrEmail, password) => {
  ...
  if (foundUser.stats) {
    localStorage.setItem(`reading_stats_${foundUser.username.toLowerCase()}`, JSON.stringify(foundUser.stats));
  }
  
  return { success: true, user: { username: foundUser.username, email: foundUser.email } };
};
